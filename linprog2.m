...
%       Edited by Ugurcan Ozalp 12.02.2019
function [x,f,it,B] = linprog2(A,b,c,k,maxit,tol)
...
Df=sign(sign(b)+.5); if k, Df(1:k)=ones(k,1); end % Full version of matrix D.
D = diag(sparse(Df));            % initial (inverse) basis matrix U.O.
...
%             disp(['LINPROG(',int2str(it),'): warning! degenerate vertex']);
            fprintf('LINPROG(%3.1f): warning! degenerate vertex', it)
         end
         Nq=min(N(r<0)); q=find(N==Nq); 
      end 
      d = D*A(:,N(q));
      ...
      if df>=0                     % apply Bland's rule to avoid cycling
         Bp=min(B(I(xbd==r))); p=find(B==Bp); 
      end 
      xb= xb - r*d; xb(p)=r;        % update x
      df=r*rmin;                    % change in f 
%       v = D(p,:) * inv(d(p)); % row vector
